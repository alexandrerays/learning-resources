...
Subimagem *nroZerosSubimagem(int width, int height, char saida[])
{
    ...
    for(i=0; i<4; i++)
    {
        fgets(line, sizeof(line), ft); //pega as 4 primeiras linhas da imagem e posiciona o ponteiro no inicio dos pixels da imagem
    }
    ...
    //percorrendo o vetor listaID de acordo com a configuracao das subimagens
    for(j=0; j<4; j++)
    {
        for(i=0; i<4; i++)
        {
...
Subimagem *analisaImagem(int a, char entrada[], char saida[])
{
    ...
    // le as dimensoes e nivel max de cinza da terceira e quarta linhas
